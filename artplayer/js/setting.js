var fosi={
    'start': function() {
        ...
        function(data) {
            if(data.code=="200"){
                fosi.url=data.url;
                fosi.type=fosi.videotype(data.url,data.type);
                $("#loading").remove();
            ...
            customType: {
                ...
                flv: function (video, url) {
                    if (flvjs.isSupported()) {
                        ...
                        fosi.ad.notice.show = '不支持播放格式：flv';
                    }
                },
            },
        });
    ...
    'DmPlayer': function() {
        fosi.danmuapi=config.api + '?ac=dm&type=xml';
        fosi.danmuapisend=config.api + '?ac=dm';
        fosi.ad = new Artplayer({
            container: '#artplayer',  /*播放器ID*/
            theme: config.themeColor,  /*进度条颜色*/
            url: fosi.url, /*视频播放地址*/
            ...
            plugins: [
                artplayerPluginDanmuku({
                    danmuku: fosi.danmuapi+'&id='+config.vkey,
        ...
        fosi.load();
        fosi.DanMu.initial();
        fosi.ad.on('artplayerPluginDanmuku:emit', (danmu) => {
            fosi.DanMu.add(danmu)
        });
    },
    'load': function() {
        fosi.nextcass();
        fosi.ad.on('video:loadedmetadata', () => {
            var currentTime = localStorage.getItem(config.vkey);
            fosi.ad.seek = currentTime;
        });
        fosi.ad.on('video:timeupdate', () => {
            var currentTime = Math.floor(fosi.ad.currentTime);
            localStorage.setItem(config.vkey,currentTime);
        });
        fosi.ad.on('video:pause', () => {
            if(config.zantingguanggaoqidong==1){
                fosi.pause.play(config.zantingguanggaolianjie, config.zantingguanggaourl);
            }
        });
        fosi.ad.on('video:play', () => {
            if(config.zantingguanggaoqidong==1){
                fosi.pause.out();
            }
        });
        fosi.ad.on('video:ended', () => {
    ...
    "DanMu" :{
        "initial":function(){
            fosi.ad.on('artplayerPluginDanmuku:error', (error) => {
                fosi.Msg("字幕加载错误",2000);
            });
            $(document).on('click', ".player-comment-setting-icon", function() {
                if (fosi.ad.plugins.artplayerPluginDanmuku.isHide) {
                    fosi.ad.plugins.artplayerPluginDanmuku.show();
                    $(this).addClass("danmu-setting-show").removeClass("danmu-setting-hide")

                } else {
                    fosi.ad.plugins.artplayerPluginDanmuku.hide();
    ...
    'Msg':function($msg,$timeout){
        ...
        $('.pop-msg').show();
        setTimeout(fosi.RemoveMsg,$timeout);
    },
